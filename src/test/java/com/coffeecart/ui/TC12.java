...
public class TC12 extends BaseTestRunner {
    ...
    public void verifyNoButtonOnAddModal() {
        ...
        Assert.assertEquals(modal.getNoButtonBorderColor(), "#daa520",
                "Border colour of 'No' button did not change on hover"
        );

        modal.clickNo();

        Assert.assertEquals(menu.getHeader().getTotalNumberItemsFromCartLink(), 0,
    ...
    public void addModalOpensAfterPageRefresh() {
        ...
        CardComponent someCard = menu.getCardByName(OTHER_DRINK);
